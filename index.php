...
if (isset($_POST['login'])) {
  ...
  $password = trim($_POST['password']); //trim: handle spasi / menghapus spasi
  $sha1 = sha1($password); //encryption

  $query = mysqli_query($config, "SELECT * FROM users WHERE email='$email'");

  //num_rows : function untuk mengecek di dalam table data query ada atau tidak
  // jika ada/berhasil akan menghasilkan nilai 1 sebaliknya akan 0
  if (mysqli_num_rows($query) > 0) {
    // mengambil data hasil query di dalam table
    $user = mysqli_fetch_assoc($query);
    // jika password yang di input user sama dengan yang ada di table
    // === : a == a, a = 1 a (var)===a(var)
